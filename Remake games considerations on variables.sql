...
--CHALLENGER REMAKE GAMES INFLUENCE ON VARIABLES
...
--GM REMAKE GAMES INFLUENCE ON VARIABLES
...
--MASTER REMAKE GAMES INFLUENCE ON VARIABLES


--Querys for counting on how many games there weren't first turret kills

SELECT COUNT(*) FROM Master_games  WHERE bluefirsttower = 0 and redfirsttower = 0
--Let's make that percentual
SELECT SUM(CASE WHEN redfirsttower = 0 and bluefirsttower = 0 THEN 1 ELSE 0 END) * 1.0/COUNT(gameId) * 100 FROM Master_games;


-- Querys for counting on how many games there weren't first blood kills
SELECT COUNT(bluefirstblood) FROM Master_games WHERE bluefirstblood = 0 and redfirstblood = 0; --TOTAL GAMES WHERE THERE WASNT FIRST BLOOD
SELECT SUM(CASE WHEN gameDuraton<220 THEN 1 ELSE 0 END) FROM Master_games WHERE redfirstblood = 0 and bluefirstblood = 0;  -- TOTAL GAMES WHERE THERE WEREN'T FIRST BLOOD AND THERE WAS REMAKE
SELECT gameId, gameDuraton, redfirstblood, bluefirstblood FROM Master_games WHERE gameDuraton > 220 and redfirstblood = 0 and bluefirstblood = 0; --GAMES WHERE THERE WERE FIRST BLOOD AND IT WASN'T REMAKE


-- Querys for counting on how many games there weren't first dragon kills
SELECT COUNT(gameId) FROM Master_games WHERE redfirstdragon = 0 and bluefirstdragon= 0;
SELECT SUM(CASE WHEN gameDuraton<220 THEN 1 ELSE 0 END) FROM Master_games WHERE redfirstdragon = 0 and bluefirstdragon = 0; -- TOTAL GAMES WHERE THERE WEREN'T FIRST BLOOD AND THERE WAS REMAKE
SELECT COUNT(*) FROM Master_games WHERE gameDuraton > 220 and redfirstdragon = 0 and bluefirstdragon = 0; --GAMES WHERE THERE WERE FIRST BLOOD AND IT WASN'T REMAKE
