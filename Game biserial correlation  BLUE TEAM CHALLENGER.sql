...
--Variables for point biserial correlation Challenger blue team

SELECT SUM(bluekills)/COUNT(bluewins) FROM Challenger_games WHERE bluewins = 1;--X1
SELECT SUM(bluekills)/COUNT(bluekills) FROM Challenger_games;--X
SELECT POWER(SUM(bluekills)/COUNT(bluekills),2) FROM Challenger_games--Xi
SELECT SQRT((POWER(SUM(bluekills),2)/COUNT(bluekills) - POWER(SUM(bluekills)/COUNT(bluekills),2))) FROM Challenger_games;--Sx
SELECT SUM(bluewins)/COUNT(bluewins) FROM Challenger_games;--p
SELECT (COUNT(bluewins)-SUM(bluewins))/COUNT(bluewins) FROM Challenger_games;--q
...
SET @x1 = (SELECT SUM(bluekills)/COUNT(bluewins) FROM Challenger_games WHERE bluewins = 1);
SET @x = (SELECT SUM(bluekills)/COUNT(bluekills) FROM Challenger_games);
SET @sx = (SELECT SQRT((POWER(SUM(bluekills),2)/COUNT(bluekills) - POWER(SUM(bluekills)/COUNT(bluekills),2))) FROM Challenger_games)
SET @p = (SELECT SUM(bluewins)/COUNT(bluewins) FROM Challenger_games)
SET @q = (SELECT (COUNT(bluewins)-SUM(bluewins))/COUNT(bluewins) FROM Challenger_games)
...
SET @x1 = (SELECT SUM(bluedragonkills)/COUNT(bluewins) FROM Challenger_games WHERE bluewins = 1);
SET @x = (SELECT SUM(bluedragonkills)/COUNT(bluedragonkills) FROM Challenger_games);
SET @sx = (SELECT SQRT((POWER(SUM(bluedragonkills),2)/COUNT(bluedragonkills) - POWER(SUM(bluedragonkills)/COUNT(bluedragonkills),2))) FROM Challenger_games)
SET @p = (SELECT SUM(bluewins)/COUNT(bluewins) FROM Challenger_games)
SET @q = (SELECT (COUNT(bluewins)-SUM(bluewins))/COUNT(bluewins) FROM Challenger_games)
SET @rpb = ((@x1 - @x)/ @sx) * SQRT(@p / @q)
...
SET @x1 = (SELECT SUM(bluebaronkills)/COUNT(bluewins) FROM Challenger_games WHERE bluewins = 1);
SET @x = (SELECT SUM(bluebaronkills)/COUNT(bluebaronkills) FROM Challenger_games);
SET @sx = (SELECT SQRT((POWER(SUM(bluebaronkills),2)/COUNT(bluebaronkills) - POWER(SUM(bluebaronkills)/COUNT(bluebaronkills),2))) FROM Challenger_games)
SET @p = (SELECT SUM(bluewins)/COUNT(bluewins) FROM Challenger_games)
SET @q = (SELECT (COUNT(bluewins)-SUM(bluewins))/COUNT(bluewins) FROM Challenger_games)
SET @rpb = ((@x1 - @x)/ @sx) * SQRT(@p / @q)
...
SET @x1 = (SELECT SUM(bluetowerkills)/COUNT(bluewins) FROM Challenger_games WHERE bluewins = 1);
SET @x = (SELECT SUM(bluetowerkills)/COUNT(bluetowerkills) FROM Challenger_games);
SET @sx = (SELECT SQRT((POWER(SUM(bluetowerkills),2)/COUNT(bluetowerkills) - POWER(SUM(bluetowerkills)/COUNT(bluetowerkills),2))) FROM Challenger_games)
SET @p = (SELECT SUM(bluewins)/COUNT(bluewins) FROM Challenger_games)
SET @q = (SELECT (COUNT(bluewins)-SUM(bluewins))/COUNT(bluewins) FROM Challenger_games)
SET @rpb = ((@x1 - @x)/ @sx) * SQRT(@p / @q)
...
SET @x1 = (SELECT SUM(bluetotalgold)/COUNT(bluewins) FROM Challenger_games WHERE bluewins = 1);
SET @x = (SELECT SUM(bluetotalgold)/COUNT(bluetotalgold) FROM Challenger_games);
SET @sx = (SELECT SQRT((POWER(SUM(bluetotalgold),2)/COUNT(bluetotalgold) - POWER(SUM(bluetotalgold)/COUNT(bluetotalgold),2))) FROM Challenger_games)
SET @p = (SELECT SUM(bluewins)/COUNT(bluewins) FROM Challenger_games)
SET @q = (SELECT (COUNT(bluewins)-SUM(bluewins))/COUNT(bluewins) FROM Challenger_games)
SET @rpb = ((@x1 - @x)/ @sx) * SQRT(@p / @q)
